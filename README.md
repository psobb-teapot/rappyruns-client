...
finished runs to the site as **drafts** via `POST /api/runs`.
"Upload to YouTube" opens the upload page plus an Explorer window with
the run's recording selected; the uploaded video's URL is then attached
to the draft on the site (My Runs → Drafts).
